...
REM Go Web Panel - 本地安装脚本 (无需GitHub下载)
REM 使用当前目录的源码进行安装
...
REM 检查源码文件
echo %BLUE%[INFO]%NC% 检查源码文件...
set MISSING_FILES=
if not exist "%CURRENT_DIR%main.go" set MISSING_FILES=!MISSING_FILES! main.go
if not exist "%CURRENT_DIR%go.mod" set MISSING_FILES=!MISSING_FILES! go.mod
if not exist "%CURRENT_DIR%system.go" set MISSING_FILES=!MISSING_FILES! system.go
if not exist "%CURRENT_DIR%camera.go" set MISSING_FILES=!MISSING_FILES! camera.go
if not exist "%CURRENT_DIR%websocket.go" set MISSING_FILES=!MISSING_FILES! websocket.go

if not "!MISSING_FILES!"=="" (
    echo %RED%[ERROR]%NC% 缺少必要文件:!MISSING_FILES!
    echo %RED%[ERROR]%NC% 请确保在项目根目录运行此脚本
    pause
    exit /b 1
)

echo %GREEN%[SUCCESS]%NC% 源码文件检查完成!
...
echo.
echo %GREEN%[SUCCESS]%NC% 本地安装完成!
echo.
echo 使用方法:
echo   启动服务: 双击桌面 "Go Web Panel" 快捷方式
echo   或运行: start.bat
echo   或直接运行: %PROJECT_NAME%.exe
...
if /i "!START_NOW!"=="y" (
    ...
    start "Go Web Panel" cmd /k "start.bat"
